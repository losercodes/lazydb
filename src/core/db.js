...
class LazyDB extends EventEmitter {
  constructor(filePath = null) {
    ...
    if (filePath) this.load(); // Only load from file if filePath is provided
  }
  ...
  async load() {
    try {
      ...
      this.data = {};
      if (this.filePath) await this.save();
    }
  }

  async save() {
    if (!this.filePath) return; // Skip save in in-memory mode
    const compressed = Compressor.compress(this.data);
    await fs.writeFile(this.filePath, compressed);
    if (this.backupPath) await fs.writeFile(this.backupPath, compressed);
  }
  ...
  update(table, id, updates) {
    if (!this.data[table] || !this.data[table][id]) throw new Error('Record not found');
...
class QueryBuilder {
  ...
  join(joinTable, leftKey, rightKey) {
    if (!this.db.data[joinTable]) throw new Error(`Join table ${joinTable} does not exist`);
  ...
  execute() {
    ...
    if (this.order) {
      results.sort((a, b) => {
        const aVal = a[this.order] ?? '';
        const bVal = b[this.order] ?? '';
        return aVal > bVal ? 1 : aVal < bVal ? -1 : 0;
      });
    }

    if (this.limitNum) results = results.slice(0, this.limitNum);
